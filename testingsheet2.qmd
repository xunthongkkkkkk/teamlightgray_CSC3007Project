...
color_palette <- c("Web" = "red", "Financial" = "red", "App" = "orange", "Transport" = "orange", "Other" = "gray")
...
gantt_plot <- ggplot(data, aes(x = StartDate, y = sector_group_num)) +
  ...
  theme(
    ...
    axis.title.y.right = element_text(size = 14, vjust = 2, face = "bold", color = "darkgreen"),
...
#geom_line(data = yearly_data, aes(x = as.Date(paste(year, "-01-01", sep = "")), y = (total_records_lost / coeff) + 1), 
           # color = "darkgreen", size = 1, alpha = 0.3, linetype = "solid") 
  geom_smooth(data = yearly_data, aes(x = as.Date(paste(year, "-01-01", sep = "")), y = (total_records_lost / coeff) + 1), method = "loess", se = FALSE,  color = "lightskyblue", size = 1, alpha = 0.3) 
